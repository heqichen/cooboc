...
ThermalArray *DeviceManager::getThermalArray(void)
{
	IDevice *thermalArrayDev = findFirstDeviceByType(DEVICE_TYPE_THERMAL_ARRAY);
	if (NULL == thermalArrayDev)
	{
		thermalArrayDev = new ThermalArray(mIo);
		thermalArrayDev->init();
		mRunningDevices[mNumRunningDevices] = thermalArrayDev;
		++mNumRunningDevices;
	}
	return (ThermalArray*)thermalArrayDev;
}
